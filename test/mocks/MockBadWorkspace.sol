...
/// @title MockBadWorkspace
/// @notice Workspace that reverts when receiving native tokens (ETH)
contract MockBadWorkspace is IWorkspace, AccountCore, ERC1271, ModuleManager {
    ...
    /// @dev Initializes the address of the {Workspace} owner, {ModuleKeeper} and enables the initial module(s)
    ...
    /// @dev Allow workspace to receive native token (ETH)
    ...
    function withdrawERC20(IERC20 asset, uint256 amount) public onlyAdminOrEntrypoint {
        // Checks: the available ERC20 balance of the workspace is greater enough to support the withdrawal
    ...
    function withdrawERC721(IERC721 collection, uint256 tokenId) public onlyAdminOrEntrypoint {
        // Checks, Effects, Interactions: withdraw by transferring the token to the workspace owner
        // Notes:
    ...
    function withdrawERC1155(
        ...
        // Checks, Effects, Interactions: withdraw by transferring the tokens to the workspace owner
        // Notes:
        // - we're using `safeTransferFrom` as the owner can be an ERC-4337 smart account
    ...
    function withdrawNative(uint256 amount) public onlyAdminOrEntrypoint {
        // Checks: the native balance of the workspace minus the amount locked for operations is greater than the requested amount
    ...
    function supportsInterface(bytes4 interfaceId) public pure returns (bool) {
        return interfaceId == type(IWorkspace).interfaceId || interfaceId == type(IERC1155Receiver).interfaceId
